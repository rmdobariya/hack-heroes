                ...
                <div class="left-signup info-pragraph">
                    ...
                    <form id="signupStore" method="post">
                        <div id="mains">
                            ...
                            $count++;
                            @endphp
                            <div id="div{{$count}}" class=" @if($loop->parent->first && $loop->first) first current @endif @if($loop->parent->last && $loop->last) last @endif">
                                <b>{!!str_replace('[]',$children,$question['question'])!!}</b>
                                <div class="text-centers">
                                    <input type="hidden" class="selected_value" name="answer[{{$key}}][{{$key1}}]">
                                    <input type="hidden" class="question" name="question[{{$key}}][{{$key1}}]" data-value="{{str_replace('[]',$children,$question['question'])}}" value="{{str_replace('[] ','',str_replace('[]','',$question['question']))}}">
                                    @foreach($question['answer'] as $key3=>$answer)
                                    <div class="form-check {{ $key3 == 0 ? 'mt-5' : '' }}">
                                        <input class="form-check-input" type="radio" name="answer[{{$key}}][{{$key1}}]" id="answer_{{$key}}_{{$key1}}_{{$key3}}" data-value="{{str_replace('[]',$children,$answer)}}" value="{{trim(str_replace('[]&apos;s','',str_replace('[] ','',str_replace('[]','',$answer))))}}" required>
